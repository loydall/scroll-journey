...
export const registerSections = (target: Element): Array<ScrollSection> => {
  ...
  return scrollSections.map((section: Element) => {
    ...
    const sectionAnimation: SectionAnimation = {
      animationInType:
        (section.getAttribute("in") as AnimationType) || AnimationType.None,
      animationOutType:
        (section.getAttribute("out") as AnimationType) || AnimationType.None,
      animationInDuration,
    ...
    const elements = sectionElements.map(
      (sectionElement: Element): ScrollElement => {
        ...
        return {
          ...
          animationInStart:
            sectionStart + Number(keyFrames[0]) * sectionPercentage,
          animationInEnd:
            sectionStart + Number(keyFrames[1]) * sectionPercentage,
          animationOutStart:
            sectionStart + Number(keyFrames[2]) * sectionPercentage,
          animationOutEnd:
            sectionStart + Number(keyFrames[3]) * sectionPercentage,
          animationInType:
