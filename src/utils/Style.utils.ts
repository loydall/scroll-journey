    ...
    switch (animationType) {
      ...
      case AnimationType.ScrollUp: 
      {
        switch (animationDirection) {
            case AnimationDirection.Out:
                {
                    element.setAttribute('style', `transform: translateY(${animationComplete * -1}lvh); will-change: transform`)
                    break
                }
            case AnimationDirection.In:
                    {
                        element.setAttribute('style', `transform: translateY(${100 - animationComplete}lvh); will-change: transform`)
                        break
                    }
                }
        break
